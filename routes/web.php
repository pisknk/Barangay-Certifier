...
// Debug utility route to reset tenant user passwords
Route::get('/admin/debug/reset-tenant-user/{tenant_id}/{new_password?}', function ($tenant_id, $new_password = null) {
    ...
    try {
        ...
        if (!$tenant) {
            return [
                'status' => 'error',
                'message' => "Tenant not found with ID: {$tenant_id}"
            ];
        }
        
        // Generate a password if not provided
        if (!$new_password) {
            $new_password = \Illuminate\Support\Str::random(8);
        }
        
        // Update password in tenant record (not hashed yet)
        ...
        // Try to update the tenant user password
        $updated = false;
        try {
            // Find the admin user with this email
            ...
            if ($user) {
                // Update the tenant_users table with the SAME hashed password
            ...
            \Illuminate\Support\Facades\Log::error("Error updating tenant user: " . $e->getMessage());
        }
        
        // End tenant context
        tenancy()->end();
        
        // Return the plain text password for the admin
        return [
            'status' => 'success',
...
Route::post('/admin/password-reset', function (\Illuminate\Http\Request $request) {
    ...
    try {
        ...
        if (!$tenant) {
            return back()->withErrors(['email' => 'No tenant found with this ID and email combination']);
        }
        ...
        if ($updated) {
            ...
            $tenantUrl = $domain ? 'http://' . $domain->domain : '#';
            ...
            return back()->withErrors(['email' => 'User record not found in tenant database']);
        }
    } catch (\Exception $e) {
...
// Diagnostic page to help debug tenant login issues
Route::get('/admin/tenant-login-debug/{tenant_id?}', function ($tenant_id = null) {
    $results = [];
    ...
    if ($tenant_id) {
        try {
            ...
            if (!$tenant) {
                return response()->json(['error' => 'Tenant not found with ID: ' . $tenant_id]);
            }
            ...
            $results['tenant_info'] = [
                ...
                'password_hash_length' => strlen($tenant->password),
                'has_plain_password_property' => property_exists($tenant, 'plain_password'),
                'is_active' => $tenant->is_active
            ];
            
            // Get domain info
            $domain = $tenant->domains()->first();
            $results['domain_info'] = $domain ? [
                'domain' => $domain->domain,
                'url' => 'http://' . $domain->domain
            ] : null;
            
            // Initialize tenant context to check user record
            tenancy()->initialize($tenant);
            ...
            $results['tenant_user_info'] = $user ? [
                ...
                'password_hash_equals_tenant' => $user->password === $tenant->password,
                'created_at' => $user->created_at
            ] : ['error' => 'No user found in tenant database with email: ' . $tenant->email];
            
            // Check if passwords match between central and tenant databases
            if ($user) {
                $results['password_comparison'] = [
                    'hashes_match' => $user->password === $tenant->password,
                    'might_be_double_hashed' => (strpos($user->password, '$2y$') === 0 && strpos($tenant->password, '$2y$') === 0)
                ];
            }
            ...
            $results['error'] = 'Error getting tenant data: ' . $e->getMessage();
        }
    } else {
        ...
        $results['available_tenants'] = $tenants;
    }
